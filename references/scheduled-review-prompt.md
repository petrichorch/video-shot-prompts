...
Prepare an English title, caption and restrained CTA for Western audiences. Save the final MP4 and a review manifest containing: reference URL, likes and duration; source-to-reconstruction shot mapping; prompts; music filename and credit; caption; output path; and any generation warnings. After the MP4 renders successfully, run `manage-reproduction-history.js --record` with the selected Douyin ID, canonical URL, author, description, likes and duration. If recording fails, report that the source has not been durably recorded.
