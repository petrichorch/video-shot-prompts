...
// Build a vertical slideshow video from generated shots and source-video audio.
const fs = require('fs');
...
if (!imagesDir || !sourceVideo || !Number.isFinite(slideSeconds) || slideSeconds <= 0) {
  console.error('Usage: create-slideshow-video.js --images-dir <outputs> --source-video <input.mp4> [--output slideshow.mp4] [--slide-seconds 2.5] [--audio-start 0]');
  process.exit(1);
}
...
if (!fs.existsSync(sourceVideo)) throw new Error(`Source video not found: ${sourceVideo}`);
...
ffmpegArgs.push('-ss', String(audioStart), '-i', sourceVideo);
...
console.log(`Creating ${images.length}-slide video (${totalSeconds.toFixed(1)}s) with audio from ${sourceVideo}`);
